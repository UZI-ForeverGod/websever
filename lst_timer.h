...
//class util_timer<http_conn>;   // 前向声明
...
public:
   time_t expire;   // 任务超时时间，这里使用绝对时间：从格林威治时间1970年01月01日00时00分00秒起至现在的总秒数
   T* task;
   util_timer<T>* prev;    // 指向前一个定时器
   util_timer<T>* next;    // 指向后一个定时器
};

// 定时器链表，它是一个升序、双向链表，且带有头指针和尾指针
template<typename T>
...
private:
    util_timer<T>* head;   // 头结点
    util_timer<T>* tail;   // 尾结点
};
